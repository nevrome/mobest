...
locate_product %>%
  ...
  coord_fixed() +
  ggtitle(paste0(
    "t for sample of interest = ", unique(locate_product$z), "\n",
    "t field time slice = ", unique(locate_product$search_z)
  ))
```
...
rnaturalearthdata::countries50 %>%
  ...
  mobest::create_prediction_grid(150000) %>%
  ggplot() + geom_raster(aes(x, y)) + coord_fixed()
```
...
- `mobest_observations_with_error_multi` (`mobest::create_obs_obserror_multi`)
...
mobest::interpolate(
  independent = positions,
  dependent = observations$ac1,
  pred_grid = expand.grid(
    x = seq(100000, 1000000, 100000), 
    y = seq(100000, 1000000, 100000)
  ) %>% { mobest::create_geopos(id = 1:nrow(.), x = .$x, y = .$y) } %>%
    mobest::geopos_to_spatpos(-5000),
  d = c(1000000, 1000000, 200)^2,
  g = 0.1
)
```
...
mul <- multiply_dependent_probabilities(locate_simple, omit_dependent_details = F)
...
mul <- multiply_dependent_probabilities(locate_overview)
