...
`mobest::crossvalidate` allows to tackle the parameter estimation challenge with simple cross-validation across a grid of kernel function paramters. Internally it already employs `mobest::create_model_grid` and `mobest::run_model_grid`.
