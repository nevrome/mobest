...
`mobest::locate_multi` produces many probability grids for each sample. Even after `mobest::multiply_dependent_probabilities` merges the per-ancestry component iterations, that still leaves many parameter permutations. `mobest::sum_probabilities_per_group` is a convenient function to sum these to a single, merged probability grid of class `mobest_locatesum`.
