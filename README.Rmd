...
```{r}
# todo:
# - locate_simple(interface that works without _multi)
# - make locate full multi-compatible (?): dependent & search_space_grid are assumed to be simple
# - option to weight different dimensions in the multiplication in multiply_dependent_probabilities?

locate_overview <- mobest::locate(
  independent = uncertain_positions,
  dependent = observations,
  kernel = mobest::create_kernset_multi(
    kernel_1 = mobest::create_kernset(
  ...
  search_time_distance = c(-35, -30, 0),
  quiet = F
)
