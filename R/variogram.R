...
calculate_all_distances <- function(independent, dependent) {
  ...
  rownames(d_obs_total) <- colnames(d_obs_total) <- dependent$id
  ...
  d_obs_long_list <- var_names %>%
    purrr::map(function(var_name) {
      ...
      rownames(d_obs) <- colnames(d_obs) <- dependent$id
      ...
      rownames(d_obs_resid) <- colnames(d_obs_resid) <- dependent$id
